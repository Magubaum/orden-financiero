...
async function cargarTodo() {
  const [movimientos, categorias, objetivos] = await Promise.all([
    api("/movimientos"), api("/categorias"), api("/objetivos"),
  ]);
...
function wireEvents() {
  ...
  document.querySelectorAll(".tab").forEach((el) => {
    el.onclick = () => { state.filtro = el.dataset.filtro; render(); };
  });
  document.querySelectorAll(".btn-del-mov").forEach((el) => {
    el.onclick = async () => { await api(`/movimientos/${el.dataset.id}`, { method: "DELETE" }); await cargarTodo(); render(); };
  });
  document.querySelectorAll(".btn-aporte").forEach((el) => {
    el.onclick = () => abrirAporte(el.dataset.id);
  });
}
...
function abrirNuevoMovimiento() {
  ...
  ov.querySelector("#mv-guardar").onclick = async () => {
    ...
    await api("/movimientos", { method: "POST", body: JSON.stringify({ tipo, categoria, monto, descripcion, fecha, persona: state.quienSos }) });
    await cargarTodo();
    ov.remove();
    render();
  };
...
function abrirCategorias() {
  ...
  function pintarChips(tipo) {
    ...
    cont.querySelectorAll(".chip-del").forEach((b) => {
      b.onclick = async () => {
        ...
        await api(`/categorias/${tipo}`, { method: "PUT", body: JSON.stringify({ categorias: next }) });
        await cargarTodo();
        pintarChips(tipo);
      };
    });
  ...
  ov.querySelector("#add-ingreso").onclick = async () => {
    ...
    await api("/categorias/ingreso", { method: "PUT", body: JSON.stringify({ categorias: [...state.categorias.ingreso, val] }) });
    await cargarTodo();
    input.value = "";
    pintarChips("ingreso");
  };
  ov.querySelector("#add-gasto").onclick = async () => {
    ...
    await api("/categorias/gasto", { method: "PUT", body: JSON.stringify({ categorias: [...state.categorias.gasto, val] }) });
    await cargarTodo();
    input.value = "";
    pintarChips("gasto");
  };
...
function abrirObjetivos() {
  ...
  function pintarLista() {
    ...
    ov.querySelectorAll(".obj-del").forEach((b) => {
      b.onclick = async () => { await api(`/objetivos/${b.dataset.id}`, { method: "DELETE" }); await cargarTodo(); pintarLista(); };
    });
  }
  pintarLista();
  ...
  ov.querySelector("#obj-agregar").onclick = async () => {
    ...
    await api("/objetivos", { method: "POST", body: JSON.stringify({ nombre, montoObjetivo: monto }) });
    await cargarTodo();
...
function abrirAporte(objetivoId) {
  ...
  ov.querySelector("#ap-guardar").onclick = async () => {
    ...
    await api(`/objetivos/${objetivoId}/aportes`, { method: "POST", body: JSON.stringify({ monto, fecha }) });
    await cargarTodo();
    ov.remove();
    render();
  };
